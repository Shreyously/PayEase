...
export function SendCard() {
  ...
  async function transferMoneyButtonClicked() {
    if (!phoneNumber || !amount) {
      alert("Enter phoneNumber and Amount first!");
    } else {
      setLoading(!loading);
      try {
        const result = await transferMoney(phoneNumber, Number(amount) * 100);
        if (result) {
          setPhoneNumber("");
          setAmount("");
        }
      } catch (error) {
        console.error("Error while money transfer!");
      } finally {
        setLoading(false);
      }
    }
        ...
        <Card title="Send Money">
          <div>
            <TextInput
              ...
              label="Phone number"
              value={phoneNumber}
              placeholder="Try send to - 999999999"
              onChange={(e) => {
            ...
            <div className="pt-10 w-full flex justify-center ">
              {loading ? (
                ...
                <Button2
                  onClick={transferMoneyButtonClicked}
                  name="Send"
